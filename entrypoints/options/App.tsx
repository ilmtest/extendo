...
const App = () => {
    ...
    const ilmTestPreviewInstance = getPreviewInstance(ilmTestApiInstance);
    const translationsPreviewInstance = getPreviewInstance(translationsApiInstance);
    ...
    return (
        <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col gap-8 p-8">
            ...
            <section className="space-y-3 rounded-lg border bg-card p-4">
                <label className="text-sm font-medium" htmlFor="translations-api-instance">
                ...
                <p className="text-xs text-muted-foreground">
                    Copy GET: {translationsPreviewInstance}
            ...
            <section className="space-y-3 rounded-lg border bg-card p-4">
                ...
                <p className="text-xs text-muted-foreground">
                    Includes background logs (heartbeat, payload gates, reconcile) persisted in storage.
                </p>
                <div className="flex gap-2 flex-nowrap">
                    <Button
                        className="w-fit min-w-[8.5rem] justify-center"
                        ...
                        onClick={async () => {
                            ...
                            anchor.href = url;
                            anchor.download = `extendo-logs-${Date.now()}.json`;
                            anchor.click();
                    ...
                    <Button
                        ...
                        variant="ghost"
                    >
                        {clearStatus === 'clearing' ? 'Clearing...' : 'Clear Logs'}
                    </Button>
                </div>
                <div className="min-h-4">
                    {exportStatus === 'no-logs' ? (
                        <p className="text-xs text-destructive">No logs captured yet.</p>
                    ) : null}
                    {exportStatus === 'exported' ? <p className="text-xs text-success">Logs downloaded.</p> : null}
                    {clearStatus === 'cleared' ? <p className="text-xs text-success">Logs cleared.</p> : null}
                </div>
            </section>
